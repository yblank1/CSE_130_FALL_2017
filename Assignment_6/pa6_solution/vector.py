...
class Vector(object):
    ...
    def __radd__(self, nextSeq):
        """ This function adds supports add when the left arg is of a differnt
type""" 
    ...
    def dot(self, next_seq):
        ...
        if isinstance(next_seq, (list, tuple, str, bytearray, buffer,
xrange, unicode, Vector)) == False:
            raise ValueError ("Error! Arg must be of type sequence") 
    ...
    def check_index(self, index, error=IndexError, get_only = True):
        """ A helper function to check if the indices are in bounds """
    ...
    def __getitem__(self, index):
        """ Gets an item in the vector using list indexing """
        if isinstance(index, slice):
    ...
    def __setitem__(self, index, value):
        """ Sets an item in the vector using string indexing"""
    ...
    def gt_ge_helper(self, vect, ge = False):
        """ Helper functino for >= and > functions. """
    ...
    def __ge__(self, vect):
        """ Checks whether a vector is greater than or equal to another vector,
by checking the consecutive highest elements of each vector"""
        return self.gt_ge_helper(vect, True)   
 
    def __le__(self, vect):
        """ Checks whether a vector is less than another vector"""
